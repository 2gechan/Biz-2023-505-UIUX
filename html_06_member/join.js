...
document.addEventListener("DOMContentLoaded", () => {
  const join = () => {
    ...
    // 특수문자, 영문대소문자, 숫자 포함하여 8 ~ 15이내
    const password_check =
      /^.*(?=^.{8,15}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/;
    if (!password_check.test(input_password)) {
      alert(
        "비밀번호는 특수문자, 영문대소문자, 숫자 포함하여 8 ~ 15자 형식으로 입력하세요"
      );
      join_inputs[INPUT_INDEX.PASSWORD].focus();
      return false;
    }
    ...
    const tel_cehck = /^\d{3}-\d{3,4}-\d{4}$/;
    if (!tel_cehck.test(input_tel)) {
